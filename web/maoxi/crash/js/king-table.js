    ...
    function table(options) {
                    ...
                    for (var i = 0; i < len; i++) {
                        if (tempDaTa[i]) {
                            ...
                            $tr.click((function(index){
                            	return function(){
                            		window.location.href="crash_analysis.html?hashCode="+tempDaTa[index].hashCode 
                            	}
                            })(i))
                            if (i % 2 == 1) {
                                $tr.addClass("evenrow");
                            ...
                            for (var key in tempDaTa[i]) {
                                if (key === "hashCode") { //隐藏值，一般用来得到表中的id，用来对单条数据进行更新或删除操作
                                    continue;
                                }
        ...
        this.showFoot = function() {
                if (this.display_tfoot) {
                    ...
                    if (this.tableObj.find("tfoot").length > 0) {
                        this.tableObj.find("tfoot").remove();
                    }
